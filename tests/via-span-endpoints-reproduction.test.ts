...
test("keeps power straight outside the via span for endpoints", async (): Promise<void> => {
  const input = createViaSpanProblem("endpoints");
  ...
  await expect(
    getViaSpanGraphics(input, output, "endpoints"),
